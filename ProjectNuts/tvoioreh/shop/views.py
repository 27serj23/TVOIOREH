...
from .services import (
    ...
    delete_order_item, get_or_create_profile, get_user_orders
)
...
def cart_view(request):
    """Просмотр корзины."""
    cart = request.session.get('cart', {})
    _, items, total = get_cart_data(cart)
...
def profile(request):
    ...
    return render(request, 'shop/profile.html', {'orders_with_items': orders_with_items, 'profile': profile})
...
def delete_order_item_view(request, item_id):
    ...
    success, msg = delete_order_item(item_id, request.user)
    if success:
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            # Заполняем профиль через сервис (бизнес-операция)
